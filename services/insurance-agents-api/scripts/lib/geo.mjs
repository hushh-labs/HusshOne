...
export const GEO_PRECISION = {
  ROOFTOP: "rooftop",
  // Provider-geocoded from a street address (Yext). Usually near-rooftop, but we cannot
  // prove rooftop per record, so it is labelled distinctly and treated as approximate.
  GEOCODED: "geocoded",
...
export function presentDistance(meters, precision) {
  ...
  // Rooftop and provider-geocoded points are precise enough to show to the metre. Only the
  // ZIP-centroid case (where many buildings share one coordinate) is floored to 500 m.
  if (precision === GEO_PRECISION.ROOFTOP || precision === GEO_PRECISION.GEOCODED) return Math.round(meters);
