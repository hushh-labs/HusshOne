#!/usr/bin/env bash
# Health + progress check for the healthcare-directory VM.
...
gcloud compute ssh "$VM_NAME" --project "$PROJECT" --zone "$ZONE" --command '
  ...
  echo
  echo "== units =="
  sudo systemctl is-active cloud-sql-proxy healthcare-directory-worker healthcare-directory-api healthcare-directory-report.timer || true
'
