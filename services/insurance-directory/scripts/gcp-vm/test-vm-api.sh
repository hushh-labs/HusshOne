#!/usr/bin/env bash
# Health + progress check for the insurance-directory VM.
...
gcloud compute ssh "$VM_NAME" --project "$PROJECT" --zone "$ZONE" --command '
  ...
  echo
  echo "== units =="
  sudo systemctl is-active cloud-sql-proxy insurance-directory-worker insurance-directory-api insurance-directory-report.timer || true
'
