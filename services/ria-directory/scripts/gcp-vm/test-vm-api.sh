#!/usr/bin/env bash
# Health + progress check for the ria-directory VM.
...
gcloud compute ssh "$VM_NAME" --project "$PROJECT" --zone "$ZONE" --command '
  ...
  echo
  echo "== units =="
  sudo systemctl is-active cloud-sql-proxy ria-directory-worker ria-directory-api ria-directory-report.timer || true
'
