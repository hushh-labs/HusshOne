...
// ON CONFLICT, so it's safe to run on every deploy. Streams the file line-by-line
// (~42k rows) and inserts in batches to keep memory flat and round-trips low.
//
// `zips` is a pure geo-reference table here: firms/advisers geo-tag their main-office
// ZIP against it. There is no per-ZIP crawl (RIA is a bulk registry ingest).
//
