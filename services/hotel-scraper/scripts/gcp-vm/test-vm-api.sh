#!/usr/bin/env bash
# Health + progress check for the hotel-scraper VM.
...
gcloud compute ssh "$VM_NAME" --project "$PROJECT" --zone "$ZONE" --command '
  ...
  echo
  echo "== units =="
  sudo systemctl is-active cloud-sql-proxy hotel-scraper-worker hotel-scraper-api hotel-scraper-report.timer || true
'
