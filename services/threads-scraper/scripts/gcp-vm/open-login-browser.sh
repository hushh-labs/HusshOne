...
gcloud compute ssh "$VM_NAME" --project "$PROJECT" --zone "$ZONE" --command \
  "sudo systemctl stop threads-scraper-api; sudo systemctl restart threads-login-browser; sudo systemctl start threads-scraper-api; sudo systemctl status --no-pager threads-login-browser | tail -20"

cat <<EOF

Threads login browser is open inside the VM display.
...
After Threads login is done, leave the Chrome window open and retry /scrape.
