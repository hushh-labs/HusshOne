...
export async function POST(request: Request) {
  try {
    ...
    const normalizedUrl = normalizeThreadsUrl(body.url);
    const profile = normalizedUrl ? buildThreadsHandshakeProfile(normalizedUrl) : null;
    if (!normalizedUrl || !profile) {
      throw Object.assign(new Error("Paste a valid Threads profile URL (e.g. https://www.threads.com/@username)."), {
        statusCode: 422,
      });
    }
